...
## Dos perfiles: minimal y full

El workspace tiene dos archivos de configuración de MCPs y vos elegís
cuál usar según el momento:

- **`.claude/mcp-minimal.json`** — solo `mariadb-dev` y `context7`. Es el
  default recomendado para el día a día. Más liviano en tokens (~3.000-5.000
  menos por sesión).
- **`.claude/mcp.json`** — todos los MCPs: `mariadb-dev`, `mariadb-prod`,
  `playwright`, `context7`, `sequential-thinking`, `granola`. Usalo cuando
  necesites alguno de los MCPs extra.

### Cuándo usar cada uno

**Usá `mcp-minimal.json` cuando**:
- Estás haciendo desarrollo normal sin tareas complejas.
- No necesitás consultar prod.
- No vas a hacer E2E con Playwright.
- No vas a procesar transcripciones de Granola.
- Estás escribiendo, revisando código, o consultando dominio.

**Usá `mcp.json` (full) cuando**:
- Necesitás consultar prod (mariadb-prod).
- Vas a hacer E2E o validación visual (playwright).
- Vas a procesar reuniones (granola).
- Tenés una tarea de razonamiento complejo donde sequential-thinking ayuda.
- Vas a hacer un post-mortem (necesitás varios MCPs juntos).

### Cómo cambiar entre uno y otro

Claude Code lee `.claude/mcp.json` por default. Para usar el minimal,
renombralo:

```bash
# Para activar minimal:
mv .claude/mcp.json .claude/mcp-full.json
mv .claude/mcp-minimal.json .claude/mcp.json

# Para volver al full:
mv .claude/mcp.json .claude/mcp-minimal.json
mv .claude/mcp-full.json .claude/mcp.json
```

O un script de switch en `scripts/switch-mcp.sh` (TODO del workspace).

**Recomendación**: empezá con minimal como default. Cambiá a full solo
cuando una sesión específica lo necesite, y volvé a minimal después.
